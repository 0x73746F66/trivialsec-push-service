...
for old_instance_id in "${old_instances_query[@]}"; do
    old_targets="${old_targets} Id=${old_instance_id},Port=${SOCKETS_PORT}"
...
readonly app_tags="[{Key=Name,Value=Sockets},{Key=Environment,Value=${TAG_ENV}},{Key=Purpose,Value=${TAG_PURPOSE}},{Key=cost-center,Value=${COST_CENTER}}]"
...
for instance in "${results[@]}"; do
    new_targets="${new_targets} Id=${instance},Port=${SOCKETS_PORT}"
